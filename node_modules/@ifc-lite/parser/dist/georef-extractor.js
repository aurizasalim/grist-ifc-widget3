 ...
 */
export function computeAngleToGridNorth(xAxisAbscissa, xAxisOrdinate) {
...
export function withGeoreferenceFallbacks(info, entities, entitiesByType) {
 ...
 */
export function extractIfc4Georeferencing(entities, entitiesByType) {
    const info = { hasGeoreference: false };
    ...
    if (projectedCRSIds.length > 0) {
        ...
        if (entity) {
            ...
            if (info.projectedCRS.name.trim() !== '')
                info.hasGeoreference = true;
        }
    }
...
function extractLegacySiteGeoreference(entities, entitiesByType) {
 ...
 */
function resolveMeasureWithUnit(ref, resolveEntity) {
    const measureRef = getReference(ref);
    if (measureRef === undefined)
        return undefined;
...
function extractProjectedCRS(entity, resolveEntity) {
