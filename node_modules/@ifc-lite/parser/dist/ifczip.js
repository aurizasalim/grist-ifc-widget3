 ...
 */
export async function unwrapIfcZip(buffer) {
 ...
 */
export async function unwrapIfcZipWithLimit(buffer, maxUncompressedBytes) {
...
async function openZipModelEntry(buffer, maxUncompressedBytes) {
    ...
    const candidates = Object.values(zip.files).filter((entry) => !entry.dir && isModelEntry(entry.name));
    if (candidates.length === 0) {
        throw new Error('This .ifcZIP archive contains no .ifc/.ifcxml entry — nothing to parse.');
    }
    if (candidates.length > 1) {
        throw new Error(`This .ifcZIP archive contains ${candidates.length} model files ` +
    ...
    if (typeof uncompressedSize === 'number' && uncompressedSize > maxUncompressedBytes) {
        throw new Error(`This .ifcZIP archive's model entry "${entry.name}" declares an uncompressed ` +
 ...
 */
export async function unwrapIfcZipView(view) {
...
//# sourceMappingURL=ifczip.js.map