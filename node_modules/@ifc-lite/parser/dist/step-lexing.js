...
export function skipComment(buf, pos, len) {
    let p = pos + 2;
    while (p + 1 < len) {
        if (buf[p] === STAR && buf[p + 1] === SLASH)
            return p + 2;
        p++;
    }
    return -1;
...
export function skipStringLiteral(buf, pos, len) {
    let p = pos + 1;
    while (p < len) {
        if (buf[p] === QUOTE) {
...
export function isKeywordLeadByte(b) {
...
export function upperKeywordByte(b) {
