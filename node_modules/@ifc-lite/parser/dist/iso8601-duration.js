...
export function parseIso8601Duration(value) {
    ...
    const magnitude = (y ? parseFloat(y) * yearSec : 0) +
...
export function secondsToIso8601Duration(seconds) {
    ...
    if (Number.isInteger(abs)) {
        if (abs % 86_400 === 0)
            return `${sign}P${toPlainDecimalString(abs / 86_400)}D`;
        if (abs % 3_600 === 0)
            return `${sign}PT${toPlainDecimalString(abs / 3_600)}H`;
        if (abs % 60 === 0)
            return `${sign}PT${toPlainDecimalString(abs / 60)}M`;
