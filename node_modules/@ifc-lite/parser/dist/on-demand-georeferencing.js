...
import { extractIfc4Georeferencing, withGeoreferenceFallbacks, } from './georef-extractor.js';
...
function computeGeoreferencingOnDemand(store) {
    ...
    const entities = entityMap; // same shape as IfcEntity
    ...
    if (!ifc4.hasGeoreference) {
        ...
        if (psetIds?.length) {
            ...
            for (const id of psetIds) {
                ...
                if (!ref)
                    continue;
                if (source.isResident && !mayContainGeorefName(source.slice(ref.byteOffset, ref.byteOffset + ref.byteLength)))
                    continue;
                const entity = extractor.extractEntity(ref);
 ...
 */
function mayContainGeorefName(bytes) {
...
//# sourceMappingURL=on-demand-georeferencing.js.map