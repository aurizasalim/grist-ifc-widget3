 ...
 */
function isBareEnumToken(token) {
...
export class EntityExtractor {
    ...
    extractEntity(ref) {
        try {
            const entityText = this.source.decodeUtf8(ref.byteOffset, ref.byteOffset + ref.byteLength);
            ...
            if (enumAttrIndices)
                entity.enumAttrIndices = enumAttrIndices;
            return entity;
        }
        catch (error) {
    ...
    parseAttributes(paramsText) {
        ...
        for (let i = 0; i < paramsText.length; i++) {
            ...
            if (char === "'") {
                ...
                if (isBareEnumToken(token))
                    (enumAttrIndices ??= []).push(attributes.length);
