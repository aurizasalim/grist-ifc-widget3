...
import { StringTable, RelationshipGraphBuilder, RelationshipType, QuantityTableBuilder, safeUtf8Decode, } from '@ifc-lite/data';
...
export { encodeIfcxImage, IFCX_APPEARANCE, IFCX_IMAGE, IFCX_APPEARANCE_SCHEMAS } from './appearance-wire.js';
export { composeIfcx, findRoots, getDescendants } from './composition.js';
export { applyTombstones, isTombstoned } from './tombstones.js';
export { bakeLayers } from './bake.js';
export { canonicalStringify, canonicalizeLayer, computeLayerId, computeStackHash, blake3Digest, } from './canonical.js';
export { PROVENANCE_VERSION, createProvenanceManifest, getProvenance, setProvenance, validateProvenance, } from './provenance.js';
...
export { extractPointClouds, } from './pointcloud-extractor.js';
...
function buildQuantities(composed, pathToId, strings) {
    ...
    const getQuantityType = (propName, qtoSet) => {
        // Volume types
        ...
        if (LENGTH_NAME_RE.test(propName))
...
export async function parseFederatedIfcx(files, options = {}) {
    ...
    options.onProgress?.({ phase: 'compose', percent: 100 });
    return finalizeFederatedResult(layerStack, compositionResult, totalSize, startTime, options);
}
/**
 ...
 */
function finalizeFederatedResult(layerStack, compositionResult, totalSize, startTime, options) {
