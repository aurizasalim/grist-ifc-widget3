 ...
 */
/**
 * Recursion guard: legitimate IFCX layers nest a handful of levels; a
 * deliberately deep JSON body (which survives `JSON.parse`) must produce a
 * clean error, not a stack-overflow `RangeError` mid-hash.
 */
const MAX_CANONICAL_DEPTH = 512;
export function canonicalStringify(value, depth = 0) {
    if (depth > MAX_CANONICAL_DEPTH) {
        throw new Error(`Value nested deeper than ${MAX_CANONICAL_DEPTH} levels is not canonicalizable`);
    }
    ...
    switch (typeof value) {
        ...
        case 'object': {
            ...
            const keys = Object.keys(record)
                ...
                .sort();
            const parts = keys.map((key) => `${JSON.stringify(key.normalize('NFC'))}:${canonicalStringify(record[key], depth + 1)}`);
            return `{${parts.join(',')}}`;
        }
        default:
 ...
 */
export function canonicalizeLayer(file) {
    ...
    const canonical = `{"data":[${data.map((entry) => entry.text).join(',')}],` +
...
function stripDerivedAttributes(node) {
    if (!node.attributes)
        return node;
    const hasDerived = Object.keys(node.attributes).some((key) => key.startsWith(IFCLITE_ATTR.DERIVED));
    if (!hasDerived)
        return node;
    const attributes = {};
 ...
 */
export function computeStackHash(layerIds) {
...
export function blake3Digest(content) {
