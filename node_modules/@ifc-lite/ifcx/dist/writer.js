...
import { IFCX_VERSION, IfcTypeEnumToString } from '@ifc-lite/data';
// ============================================================================
...
export class IfcxWriter {
    ...
    collectNodes(options) {
        ...
        const resolvedPaths = new Map(idToPath ?? []);
        for (let i = 0; i < entities.count; i++) {
            const expressId = entities.expressId[i];
            const typeEnum = entities.typeEnum[i];
            resolvedPaths.set(expressId, idToPath?.get(expressId)
     ...
     */
    getChildrenForEntity(entityId, spatialHierarchy, resolvedPaths) {
        const children = {};
        if (!spatialHierarchy)
            return children;
     ...
     */
    getTypeName(expressId, typeEnum) {
        const table = this.data.entities;
        const fromTable = table.getTypeName?.(expressId);
        // 'Unknown' is the table's own miss sentinel, not an IFC class.
        if (fromTable && fromTable !== 'Unknown')
            return fromTable;
        const fromEnum = IfcTypeEnumToString(typeEnum);
     ...
     */
    generatePath(expressId, typeEnum, globalId) {
        if (globalId)
            return globalId;
