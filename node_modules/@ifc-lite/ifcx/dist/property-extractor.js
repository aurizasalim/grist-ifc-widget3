            ...
            for (const [system, refs] of bySystem) {
                ...
                for (const ref of refs) {
                    ...
                    if (ref.uri !== undefined)
                        classificationProps.set('Uri', ref.uri);
                    if (ref.description !== undefined)
                        classificationProps.set('Description', ref.description);
                }
            }
            continue;
        }
 ...
 */
export function routesToQuantityTable(key, value) {
    const effective = isTypedPropertyValue(value) ? value.value : value;
    if (typeof effective !== 'number')
        return false;
    const v5a = parseV5aKey(key);
    if (v5a) {
        if (v5a.setName.startsWith('Pset_'))
            return false;
        if (v5a.setName.startsWith('Qto_'))
            return true;
