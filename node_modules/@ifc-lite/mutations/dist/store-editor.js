...
export class StoreEditor {
    ...
    forkPreparedEditor(view) {
        ...
        const editor = Object.create(StoreEditor.prototype);
     ...
     */
    setEntityType(expressId, newType, options) {
        ...
        if (!/^[Ii][Ff][Cc][A-Za-z][A-Za-z0-9_]*$/.test(trimmed)) {
            throw new Error(`StoreEditor.setEntityType: newType "${newType}" is not a recognizable IFC entity name (expected e.g. "IfcColumn")`);
        }
        ...
        if (configuredNormalizer) {
            const resolved = configuredNormalizer(trimmed);
            if (!resolved) {
                throw new Error(`StoreEditor.setEntityType: newType "${newType}" is not in the IFC schema registry (typo? vendor extension?)`);
            }
            canonical = resolved;
        }
     ...
     */
    addQuantitySet(entityId, qsetName, quantities) {
        const kind = {
        ...
        this.view.createQuantitySet(entityId, qsetName, quantities.map((q) => ({ name: q.name, value: q.value, quantityType: kind[q.quantityType], unit: q.unit })));
    }
    /**
     ...
     */
    addPropertySet(entityId, psetName, properties) {
        const kind = {
        ...
        this.view.createPropertySet(entityId, psetName, properties.map((p) => ({ name: p.name, value: p.value, type: kind[p.type], unit: p.unit })));
    }
