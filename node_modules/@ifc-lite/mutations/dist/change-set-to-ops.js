...
export function changeSetToOps(changeSet, resolver) {
    ...
    for (const [entity, perEntity] of components) {
        if (entityOps.get(entity)?.op === 'tombstone-entity')
            continue;
...
function applyMutation(mutation, entity, setMember, componentFor, entityOps, resolver, skipped) {
    switch (mutation.type) {
        ...
        case 'UPDATE_QUANTITY':
            if (mutation.psetName && mutation.propName) {
                ...
                for (const q of mutation.newValue) {
        ...
        case 'CREATE_PROPERTY_SET':
            if (mutation.psetName) {
                ...
                if (Array.isArray(mutation.newValue)) {
                    for (const prop of mutation.newValue) {
