...
function stringifyEffectiveValue(value) {
...
function compareEffectiveChanges(a, b) {
...
function collectAttributeAndTypeChanges(snapshot, resolvers, changes) {
    for (const [key, mutation] of snapshot.attributeMutations) {
        ...
        const previousValue = resolvers.attributeExtractor
            ? stringifyEffectiveValue(resolvers.attributeExtractor(resolvers.resolveBaseEntityId(entityId), mutation.attribute))
...
function collectSetLevelChanges(snapshot, changes) {
 ...
 */
function resolveMutation(mutations, key) {
 ...
 * here.
 */
function collectPropertyChanges(snapshot, resolvers, changes) {
    for (const entityId of snapshot.propertyKeysByEntity.keys()) {
        ...
        for (const pset of basePsets) {
            if (snapshot.deletedPsets.has(`${entityId}:${pset.name}`))
                continue;
            if (snapshot.newPsets.get(entityId)?.has(pset.name))
                continue;
            for (const prop of pset.properties) {
                const key = propertyKey(entityId, pset.name, prop.name);
                if (!entityPropKeys.has(key))
                    continue;
                visited.add(key);
            ...
            for (const key of entityPropKeys) {
                if (visited.has(key) || !key.startsWith(psetPrefix))
                    continue;
                visited.add(key);
                const mutation = resolveMutation(snapshot.propertyMutations, key);
                if (!mutation)
...
function collectQuantityChanges(snapshot, resolvers, changes) {
    for (const entityId of snapshot.quantityKeysByEntity.keys()) {
        ...
        for (const qset of baseQsets) {
            if (snapshot.deletedQsets.has(`${entityId}:${qset.name}`))
                continue;
            if (snapshot.newQsets.get(entityId)?.has(qset.name))
                continue;
            for (const q of qset.quantities) {
                const key = quantityKey(entityId, qset.name, q.name);
                if (!entityQtyKeys.has(key))
                    continue;
                visited.add(key);
            ...
            for (const key of entityQtyKeys) {
                if (visited.has(key) || !key.startsWith(qsetPrefix))
                    continue;
                visited.add(key);
                const mutation = resolveMutation(snapshot.quantityMutations, key);
                if (!mutation)
...
function collectEntityChanges(snapshot, changes) {
...
export function collectEffectiveChanges(snapshot, resolvers) {
    const changes = [];
    ...
    return changes.filter((c) => {
        if (snapshot.forgottenCreatedEntities.has(c.entityId))
            return false;
        if (c.kind !== 'entity-deleted' && snapshot.tombstones.has(c.entityId))
            return false;
        if (isNoOpChange(c))
            return false;
        return true;
...
//# sourceMappingURL=effective-changes.js.map