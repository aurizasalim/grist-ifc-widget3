...
export class MutablePropertyView extends MutableOverlayState {
    ...
    prepareAtomic(edit) {
        ...
        const validate = () => {
            if (rolledBack)
                throw new Error('The prepared IFC transaction was rolled back.');
            if (!committed && !this.matchesOverlayState(original))
                throw new Error('The IFC overlay changed during a prepared transaction.');
        };
        ...
        return { result, validate, commit: () => {
                ...
                if (!committed || rolledBack)
                    return;
                if (!this.matchesOverlayState(prepared))
                    throw new Error('The IFC overlay changed after the prepared transaction committed.');
     ...
     */
    setExpressIdWatermark(maxExistingId) {
    ...
    setPropertyMutation(entityId, key, mutation) {
    ...
    deletePropertyMutation(entityId, key) {
        ...
        if (removed) {
            ...
            if (bucket) {
                bucket.delete(key);
                if (bucket.size === 0)
                    this.propertyKeysByEntity.delete(entityId);
            }
        }
        return removed;
    }
    setQuantityMutation(entityId, key, mutation) {
    ...
    deleteQuantityMutation(entityId, key) {
        ...
        if (removed) {
            ...
            if (bucket) {
                bucket.delete(key);
                if (bucket.size === 0)
                    this.quantityKeysByEntity.delete(entityId);
            }
        }
        return removed;
    }
    setAttributeMutation(entityId, key, mutation) {
    ...
    deleteAttributeMutation(entityId, key) {
        ...
        if (removed) {
            ...
            if (bucket) {
                bucket.delete(key);
                if (bucket.size === 0)
                    this.attributeKeysByEntity.delete(entityId);
            }
        }
        return removed;
    }
    ...
    setProperty(entityId, psetName, propName, value, valueType = PropertyValueType.String, unit, skipHistory = false, dataType) {
        ...
        const maskedInSession = this.deletedPsets.has(`${entityId}:${psetName}`) ||
            this.propertyMutations.get(key)?.operation === 'DELETE';
        const propExistedBefore = oldValue !== null ||
            (!maskedInSession &&
     ...
     */
    createQuantitySet(entityId, qsetName, quantities) {
     ...
     */
    setQuantity(entityId, qsetName, quantName, value, qType = QuantityType.Count, unit, skipHistory = false) {
    ...
    deleteQuantity(entityId, qsetName, quantName, skipHistory = false) {
        return deleteQuantityMember({
            ...
            pushHistory: mutation => { if (!skipHistory)
                this.mutationHistory.push(mutation); },
     ...
     */
    deleteQuantitySet(entityId, qsetName) {
     ...
     */
    isQuantitySetDeleted(entityId, qsetName) {
     ...
     */
    setAttribute(entityId, attrName, value, oldValue, skipHistory = false) {
     ...
     * `null` becomes `$`.
     */
    setPositionalAttribute(entityId, index, value, skipHistory = false) {
    ...
    getPositionalMutationsForEntity(entityId) {
     ...
     */
    removePositionalMutation(entityId, index) {
     ...
     */
    setEntityType(entityId, newType, predefinedType, oldType, skipHistory = false) {
        ...
        if (!/^[Ii][Ff][Cc][A-Za-z][A-Za-z0-9_]*$/.test(trimmed)) {
            throw new Error(`setEntityType: "${newType}" is not a recognizable IFC entity name (expected e.g. "IfcColumn")`);
        }
    ...
    getEntityTypeMutation(entityId) {
    ...
    deleteEntity(expressId) {
        ...
        if (this.tombstones.has(expressId))
            return false;
        this.tombstones.add(expressId);
        this.mutationHistory.push({
            id: generateMutationId(),
     ...
     */
    restoreFromTombstone(expressId) {
     ...
     */
    getAttributeMutationsForEntity(entityId) {
        const result = [];
     ...
     */
    removeQuantityMutation(entityId, qsetName, quantName) {
        if (quantName) {
            const key = quantityKey(entityId, qsetName, quantName);
     ...
     */
    removeAttributeMutation(entityId, attrName) {
     ...
     */
    hasPendingChanges() {
        return (this.propertyMutations.size > 0 ||
    ...
    applyMutations(mutations) {
        applyMutationsBatch(this, mutations, (entityId) => this.newEntities.has(entityId), (entityId, qsetName) => this.deletedQsets.add(`${entityId}:${qsetName}`), (mutation, qsetName, quantName, retainHistory) => {
            this.setQuantityMutation(mutation.entityId, quantityKey(mutation.entityId, qsetName, quantName), { operation: 'DELETE' });
            if (retainHistory)
                this.mutationHistory.push(mutation);
        });
    }
