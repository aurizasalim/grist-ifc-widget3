 ...
 * not diagnostics.
 */
export function freeWasmInstanceQuietly(instance) {
    try {
        instance?.free();
    }
    catch (err) {
        if (freeFailureLogged)
            return;
        freeFailureLogged = true;
        console.warn('[Worker] Freeing the failed engine instance threw, so its file-sized source copy stays ' +
            'in the wasm heap until GC (logged once per worker):', err);
    }
}
//# sourceMappingURL=wasm-instance-free.js.map