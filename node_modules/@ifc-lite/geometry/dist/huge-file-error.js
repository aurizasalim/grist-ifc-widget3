...
const OOM_SIGNATURE = /unreachable|out of memory|memory access|RuntimeError|allocat|enlarge memory|grow memory|could not allocate/i;
/**
 ...
 */
export function largeFilePrepassError(err, byteLength) {
    const sizeGB = byteLength / 1e9;
    const raw = err instanceof Error ? err.message : String(err);
    if (sizeGB >= HUGE_FILE_GB_THRESHOLD && OOM_SIGNATURE.test(raw)) {
        return new Error(`This model is ${sizeGB.toFixed(1)} GB, which exceeds the browser's ~3 GB WebAssembly memory ` +
            `ceiling (32-bit address space). Open it in the ifc-lite desktop app, which has no such limit. ` +
            `(underlying wasm trap: ${raw})`, { cause: err });
    }
    return null;
}
//# sourceMappingURL=huge-file-error.js.map