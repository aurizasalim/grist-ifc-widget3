...
export function propertyTableFromColumns(columns, strings) {
    const { count, entityId, psetName, psetGlobalId, propName, propType, valueString, valueReal, valueInt, valueBool, unitId, } = columns;
    ...
    const table = {
        ...
        getForEntity: (id) => {
            ...
            return groupPropertySetsByInstance(rowIndices, psetName, psetGlobalId, propName, propType, strings, (idx) => getPropertyValue(table, idx, strings));
        },
        ...
        findByProperty: (prop, operator, value, pset) => {
            ...
            if (pset !== undefined && psetIdx < 0)
                return [];
            const rowIndices = propIndex.get(propIdx) || [];
            const results = [];
            for (const idx of rowIndices) {
                if (psetIdx >= 0 && psetName[idx] !== psetIdx)
                    continue;
...
export function propertyTableToColumns(table) {
    return {
        count: table.count,
...
export function __resetListParseWarningLatch() {
    listParseWarningShown = false;
}
function warnListParseFailureOnce(listStr, err) {
    if (listParseWarningShown)
        return;
    listParseWarningShown = true;
    console.warn(`[data] property list value is not valid JSON; reporting an empty list ` +
        `(further occurrences suppressed). value=${JSON.stringify(listStr.slice(0, 120))}`, err);
}
 ...
 */
export function comparePropertyValues(propValue, operator, value) {
