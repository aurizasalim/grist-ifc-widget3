 ...
 */
export function formatStepReal(value) {
 ...
 */
export function toStepLineWithRegistry(registry, entity) {
...
export function generateHeader(options) {
    const toList = (v, fallback) => v === undefined ? fallback : Array.isArray(v) ? v : [v];
    const quoteList = (items) => '(' + items.map(s => "'" + escapeStepString(s) + "'").join(',') + ')';
 ...
 */
export function generateStepFileWithRegistry(registry, entities, options) {
 ...
 */
export function parseStepValue(str) {
    str = str.trim();
    // Null
    if (str === '$') {
        return null;
 ...
 */
function unescapeStepString(str) {
    return decodeStepStringLiteral(str);
}
//# sourceMappingURL=step-serializers.js.map