...
const ALL_VERSIONS = ['IFC2X3', 'IFC4', 'IFC4X3'];
 ...
 * bucket.
 */
function renameGroup(upper) {
    const group = [upper];
    for (const pair of CROSS_SCHEMA_RENAMES) {
        if (!pair.includes(upper))
            continue;
 ...
 */
const ALIAS_ROWS = Object.entries(ENTITY_NAME_ALIASES).map(([leaf, supertype]) => [leaf.toUpperCase(), supertype.toUpperCase()]);
/**
 ...
 */
function descendantsInTable(table, seeds, own) {
    const found = new Set();
    const stack = [...seeds];
    const visited = new Set(seeds);
    while (stack.length > 0) {
        for (const child of table.children.get(stack.pop()) ?? []) {
...
function computeDescendantsOf(upper, v) {
    ...
    for (const other of ALL_VERSIONS) {
        ...
        if (table === own)
            continue;
        for (const name of descendantsInTable(table, seeds, verdict))
            found.add(name);
    }
    ...
    for (const [leaf, parent] of ALIAS_ROWS) {
        if (parent === upper || found.has(parent))
            found.add(leaf);
    }
    found.delete(upper);
 ...
 * losing an entry.
 */
export function expandTypeNamesToDescendants(types, schemaVersion) {
