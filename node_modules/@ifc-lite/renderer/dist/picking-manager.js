...
export class PickingManager {
    ...
    prepareBatchedPick(options) {
        ...
        if (batchedMeshes.length === 0 && this.scene.getTexturedMeshes().length === 0)
            return 'gpu';
        ...
        for (const expressId of expressIds) {
            if (!isEntityVisible(expressId, options?.hiddenIds, options?.isolatedIds))
                continue;
    ...
    async pick(x, y, options, clip) {
        ...
        const result = await this.picker.pick(scaledX, scaledY, this.canvas.width, this.canvas.height, meshes, viewProj, pointNodes, pointSizing, this.scene.getInstancedTemplates(), clip);
        return result;
    }
    /**
    ...
    async pickRect(x0, y0, x1, y1, options, clip) {
        ...
        const viewProj = this.camera.getViewProjMatrix().m;
        const pointSnap = this.pointPickProvider?.() ?? null;
        return this.picker.pickRect(sx0, sy0, sx1, sy1, this.canvas.width, this.canvas.height, meshes, viewProj, pointSnap?.nodes ?? undefined, pointSnap?.sizing ?? undefined, this.scene.getInstancedTemplates(), clip);
    }
}
