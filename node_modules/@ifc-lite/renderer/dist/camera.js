...
import { areFiniteNumbers, DEFAULT_ORTHO_SIZE, isUsableBounds, isUsableDistance, usableOrthoSize, } from './camera-guards.js';
...
export class Camera {
    ...
    orbit(deltaX, deltaY, addVelocity = false) {
        ...
        if (!this.controls.orbit(deltaX, deltaY))
            return;
    ...
    pan(deltaX, deltaY, addVelocity = false) {
        ...
        if (!this.controls.pan(deltaX, deltaY))
            return;
     ...
     */
    zoom(delta, addVelocity = false, mouseX, mouseY, canvasWidth, canvasHeight, fastZoom) {
        // Gate inertia on whether `zoom` applied, same reason as `orbit` above (#2934 review).
        if (!this.controls.zoom(delta, mouseX, mouseY, canvasWidth, canvasHeight, fastZoom))
            return;
        if (addVelocity) {
     ...
     */
    async applyFitPolicy(policy, duration = 500) {
        return this.animator.animateToWithUp({ ...policy.position }, { ...policy.target }, { ...policy.up }, duration);
    }
    /**
    ...
    setRotation(azimuth, elevation) {
        ...
        if (!areFiniteNumbers(azimuth, elevation))
            return;
        ...
        if (!areFiniteNumbers(target.x, target.y, target.z))
            return;
        const theta = ((((azimuth % 360) + 360) % 360) * Math.PI) / 180;
        const poleMargin = CAMERA_CONSTANTS.MIN_PHI;
        const phi = Math.max(poleMargin, Math.min(Math.PI - poleMargin, ((90 - elevation) * Math.PI) / 180));
