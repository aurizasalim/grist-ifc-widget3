 ...
 */
export function pickFitPolicy(bounds, options) {
    const center = {
    ...
    const distanceForFeature = (featureSize * viewportPx) / (2 * TARGET_FEATURE_PIXELS * tanHalfFov);
    ...
    const tiltFrom = Math.abs(along.y) > 0.5 ? { x: 0, y: 0, z: 1 } : up;
...
function pickLongestUnitAxis(sx, sy, sz) {
    if (sx >= sy && sx >= sz)
        return { x: 1, y: 0, z: 0 };
    if (sz >= sy)
        return { x: 0, y: 0, z: 1 };
...
//# sourceMappingURL=camera-fit-policy.js.map