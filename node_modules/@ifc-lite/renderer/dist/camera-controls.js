...
import { areFiniteNumbers, isUsableCursorAnchor, isUsableDistance, isUsableUp, usableOrthoSize, } from './camera-guards.js';
// ---------------------------------------------------------------------------
...
export class CameraControls {
    ...
    orbit(deltaX, deltaY) {
        ...
        if (!areFiniteNumbers(deltaX, deltaY))
            return false;
        if (this.interactionMode === 'pan' || this.interactionMode === 'none')
            return false;
    ...
    orbitAroundExternalPivot(pivot, dx, dy) {
        ...
        if (!isUsableDistance(length(look), 0))
            return;
    ...
    pan(deltaX, deltaY) {
        ...
        if (!areFiniteNumbers(deltaX, deltaY))
            return false;
        if (this.interactionMode === 'orbit' || this.interactionMode === 'none')
            return false;
     ...
     */
    zoom(delta, mouseX, mouseY, canvasWidth, canvasHeight, fastZoom) {
        ...
        if (!isUsableDistance(distance, CC.MIN_PERSPECTIVE_DISTANCE))
            return false;
     ...
     */
    zoomPerspective(distance, forward, zoomFactor, fastZoom) {
    ...
    shiftTargetTowardsMouse(dir, distance, forward, zoomFactor, mouseX, mouseY, canvasWidth, canvasHeight) {
        ...
        if (!isUsableUp(this.state.camera.up))
            return;
