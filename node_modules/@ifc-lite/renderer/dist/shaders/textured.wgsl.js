...
function replaceOnce(src, find, repl, label) {
    ...
    if (idx === -1) {
        throw new Error(`textured.wgsl: anchor "${label}" not found — main.wgsl.ts changed; update the textured-shader derivation.`);
    }
...
function deriveTexturedShader() {
    let s = mainShaderSource;
    // 1. Albedo texture + sampler bindings (same group(0) as the uniform).
    s = replaceOnce(s, '@binding(0) @group(0) var<uniform> uniforms: Uniforms;', '@binding(0) @group(0) var<uniform> uniforms: Uniforms;\n' +
        '        @binding(1) @group(0) var albedoTex: texture_2d<f32>;\n' +
        '        @binding(2) @group(0) var albedoSampler: sampler;', 'uniform binding');
    ...
    s = replaceOnce(s, '          @location(2) entityId: u32,\n        }\n\n        struct VertexOutput {', '          @location(2) entityId: u32,\n          @location(10) uv: vec2<f32>,\n        }\n\n        struct VertexOutput {', 'VertexInput uv');
    // 3. UV interpolant on VertexOutput (after the @location(5) instSelected varying).
    s = replaceOnce(s, '          @location(5) @interpolate(flat) instSelected: u32,\n        }', '          @location(5) @interpolate(flat) instSelected: u32,\n          @location(6) uv: vec2<f32>,\n        }', 'VertexOutput uv');
    // 4. Pass UV through the vertex shader.
    s = replaceOnce(s, '          output.entityId = input.entityId;', '          output.entityId = input.entityId;\n          output.uv = input.uv;', 'vs_main uv passthrough');
    ...
    s = replaceOnce(s, 'var baseColor = input.color.rgb;', 'let albedoTexel = textureSample(albedoTex, albedoSampler, input.uv);\n' +
        '          if (albedoTexel.a < 0.004) { discard; }\n' +
        '          var baseColor = albedoTexel.rgb * input.color.rgb;', 'albedo sample');
    return s;
}
export const texturedShaderSource = deriveTexturedShader();
//# sourceMappingURL=textured.wgsl.js.map