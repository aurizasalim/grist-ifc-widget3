 ...
 */
export function composeInstanceMatrix(transformRowMajor, origin) {
 ...
 */
export function writeInstanceRecord(dv, byteOffset, instanceMatrix, entityId, color, flags = 0) {
    for (let j = 0; j < 16; j++) {
 ...
 */
export function prepareInstancedRender(shard) {
    const byTemplate = shard.templates.map(() => []);
    for (const inst of shard.instances) {
        // Glass/transparent → flat transparent pipeline, not the opaque instanced one.
        if (inst.color[3] < OPAQUE_ALPHA_CUTOFF)
            continue;
 ...
 */
export function foldOccurrenceWorldBox(meta, w) {
    if (meta.maxOccRadius === Infinity)
        return; // sticky poison
    const dx = w.maxX - w.minX, dy = w.maxY - w.minY, dz = w.maxZ - w.minZ;
    const occRadius = 0.5 * Math.sqrt(dx * dx + dy * dy + dz * dz);
    if (!Number.isFinite(occRadius) ||
    ...
    if (occRadius > meta.maxOccRadius)
        meta.maxOccRadius = occRadius;
...
//# sourceMappingURL=instanced-render.js.map