 ...
 */
import { clipIsActive, boxFullyClipped } from './scene-raycaster.js';
/**
 ...
 */
export function selectBoundingBoxesInRect(boundingBoxes, viewProj, rect, viewportWidth, viewportHeight, hiddenIds, isolatedIds, clip) {
    const hits = new Set();
    ...
    for (const [expressId, bbox] of boundingBoxes) {
        ...
        if (!projected)
            continue;
        if (maxX < rx0 || minX > rx1 || maxY < ry0 || minY > ry1)
            continue;
        hits.add(expressId);
    }
    return hits;
}
//# sourceMappingURL=scene-rect-select.js.map