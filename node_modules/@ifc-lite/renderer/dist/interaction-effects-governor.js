...
export class InteractionEffectsGovernor {
    lastInteractiveTs = null;
    deltas = [];
    degraded = false;
    strikes = 0;
    lastStrikeTs = -Infinity;
    cleanStreak = 0;
    /**
     ...
     */
    frame(interacting, now, unstable = false, expectedIntervalMs = 0) {
        if (!interacting) {
        ...
        if (last === null || now - last > BURST_GAP_MS) {
            ...
            this.cleanStreak = 0;
            if (this.degraded) {
                const cooldown = Math.min(REPROBE_BASE_MS * Math.pow(2, Math.max(0, this.strikes - 1)), REPROBE_MAX_MS);
        ...
        if (!this.degraded) {
            ...
            const refresh = Math.min(Math.max(windowMin, REFRESH_MIN_MS), REFRESH_MAX_MS);
...
//# sourceMappingURL=interaction-effects-governor.js.map