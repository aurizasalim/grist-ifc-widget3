...
export function simplifyIndicesByClustering(vertexData, strideFloats, indices, cellSize) {
    const triangleCount = (indices.length / 3) | 0;
    if (triangleCount < LOD_MIN_TRIANGLES)
        return null;
    ...
    const repOfCell = new Map(); // exact cell key -> representative vertexIndex
    let cellOrigin;
    ...
    const repOf = (vi) => {
        ...
        if (existing === undefined) {
            ...
            if (repOfCell.size >= MAX_MAP_ENTRIES)
                return -1;
...
export function lodCellSizeForBounds(min, max) {
    const dx = max[0] - min[0];
    const dy = max[1] - min[1];
    const dz = max[2] - min[2];
    return Math.sqrt(dx * dx + dy * dy + dz * dz) * LOD_CELL_FRACTION;
}
//# sourceMappingURL=lod-simplify.js.map