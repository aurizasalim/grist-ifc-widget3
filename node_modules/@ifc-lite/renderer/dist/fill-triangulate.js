...
function pointOnRing(p, ring) {
    for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
        if (pointOnSegment(p, ring[i], ring[j], true))
            return true;
    }
    return false;
}
 ...
 */
export function groupRingsByNesting(rings) {
    const usable = rings.filter((r) => r.length >= 3).map((r) => r.slice());
    const depth = usable.map((r, i) => usable.reduce((d, other, j) => (j !== i && ringInsideRing(r, other) ? d + 1 : d), 0));
    const groups = [];
    for (let i = 0; i < usable.length; i++) {
        ...
        for (let j = 0; j < usable.length; j++) {
            if (j === i || depth[j] !== depth[i] + 1)
                continue;
            if (ringInsideRing(usable[j], usable[i]))
                holes.push(usable[j]);
        }
...
export function earClip(ring) {
    ...
    while (indices.length > 3 && safety-- > 0) {
        ...
        if (degenerate < 0)
            break; // see the note above on partial output
...
function isEar(ring, indices, i, scale) {
    const m = indices.length;
    ...
    for (let j = 0; j < m; j++) {
        ...
        if (ij === ia || ij === ib || ij === ic)
            continue;
        const p = ring[ij];
        if (samePoint(p, a, scale) || samePoint(p, b, scale) || samePoint(p, c, scale))
            continue;
        const prev = ring[indices[(j + m - 1) % m]];
        const next = ring[indices[(j + 1) % m]];
        if (orient(prev, p, next) > 0)
            continue; // strictly convex — cannot obstruct
        if (pointInTriangle(p, a, b, c))
            return false;
    }
    return true;
}
function findZeroAreaVertex(ring, indices) {
    const m = indices.length;
    for (let i = 0; i < m; i++) {
...
export function triangulateRings(rings) {
    ...
    for (const group of groupRingsByNesting(rings)) {
        const stitched = group.holes.length === 0 ? group.outer : joinHoles(group.outer, group.holes);
...
//# sourceMappingURL=fill-triangulate.js.map