...
import { ENVIRONMENT_UNIFORM_SIZE, packEnvironmentUniforms, resolveEnvironment, } from './environment.js';
/**
...
export class RenderPipeline {
    ...
    webgpuDevice;
    pipeline;
    instancedPipeline; // GPU-instancing: template (slot 0) + per-instance buffer (slot 1)
    instancedTransparentPipeline = null; // instanced pipeline with alpha blend (lens/x-ray/compare overlays); lazily built, null if unbuilt/rejected
    makeInstancedTransparentPipeline = null; // deferred factory (see constructor)
    makeQuantizedPipelineAsync = null;
    quantizedPipelines = {};
    instancedTransparentPipelineTried = false; // built-or-failed once; don't retry a rejecting backend every frame
    ...
    overlayPipeline; // Pipeline for color overlays (lens) - renders at exact same depth
    texturedPipeline; // Pipeline for textured meshes (#961): UV lane + albedo texture/sampler
    texturedBindGroupLayout; // group(0): uniform + texture + sampler
    depthTexture;
    depthTextureView;
    ...
    environmentBuffer;
    environmentBindGroup;
    environmentBindGroupLayout;
    environmentScratch = new Float32Array(ENVIRONMENT_UNIFORM_SIZE / 4);
    ...
    constructor(device, width = 1, height = 1) {
        ...
        this.makeInstancedTransparentPipeline = () => this.device.createRenderPipeline({
        ...
        this.makeQuantizedPipelineAsync = (kind) => {
            const base = kind === 'opaque' ? pipelineDescriptor :
     ...
     */
    updateUniforms(viewProj, model, color, material, sectionPlane, isSelected, clipBox) {
     ...
     */
    async ensureQuantizedPipelines() {
        const factory = this.makeQuantizedPipelineAsync;
        if (!factory)
            return false;
        const kinds = ['opaque', 'transparent', 'overlay'];
        try {
    ...
    getQuantizedPipelineVariant(kind) {
        return this.quantizedPipelines[kind] ?? null;
    }
    writeRawUniforms(data, extraFlagsX = 0) {
        ...
        if (extraFlagsX !== 0) {
            ...
            this.device.queue.writeBuffer(this.uniformBuffer, 176, new Uint32Array([baseFlagsX | extraFlagsX]));
        }
    }
    /**
    ...
    getInstancedTransparentPipeline() {
        ...
        if (!this.instancedTransparentPipeline &&
     ...
     */
    createTexturedBindGroup(uniformBuffer, textureView, sampler) {
        return this.device.createBindGroup({
