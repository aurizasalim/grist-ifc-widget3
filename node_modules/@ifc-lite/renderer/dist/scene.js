...
import { inheritedQuantization, groupOverridePieces, cloneOverrides, overlaysInvalidatedBy } from './scene-derived-batches.js';
...
export class Scene {
    ...
    instancedOverrideColors = null;
    lastGhostAlpha = 1; // alpha the active X-Ray fade was written with
    ...
    instancedGhostDirty = false;
    instancedHasTransparent = false; // an override made some instanced occurrence translucent
    instancedGhostTransparent = false; // X-Ray ghosting made some instanced occurrence translucent
    ...
    overrideBatches = [];
    overlaySources = new Map(); // overridden piece -> bucket key its overlay inherited from (#4832)
     ...
     */
    getMeshData(expressId, modelIndex) {
    ...
    rebuildPendingBatches(device, pipeline) {
        ...
        if (!this.finalizeInProgress && overlaysInvalidatedBy(rebuilt, this.overlaySources))
    ...
    removeMeshesForEntity(expressId) {
        ...
        for (const meshData of meshDataList) {
            ...
            if (hostsOtherEntities(meshData))
                continue;
     ...
     */
    removeMeshesForEntities(expressIds) {
        let count = 0;
        for (const id of expressIds) {
            if (this.removeMeshesForEntity(id))
                count++;
        }
        return count;
    }
    ...
    translateMeshesForEntities(updates) {
        ...
        for (const [id, delta] of updates) {
            if (this.translateMeshesForEntity(id, delta))
                count++;
        }
        return count;
    }
     ...
     */
    createBatchedMesh(meshes, color, device, pipeline, bucketKey, quantization = this.quantizedBatchesEnabled ? 'auto' : 'off') {
    ...
    getOrCreatePartialBatch(sourceBatchKey, colorKey, visibleIds, device, pipeline, visibilityEpoch) {
        ...
        const partialBatch = this.createBatchedMesh(visibleMeshData, color, device, pipeline, undefined, inheritedQuantization(this.quantizedBatchesEnabled, bucket?.batchedMesh));
        // Cache it
    ...
    setColorOverrides(overrides, device, pipeline) {
        ...
        const colorGroups = groupOverridePieces(overrides, (id) => this.meshDataMap.get(id), (piece) => this.meshDataBucket.get(piece), (piece) => this.bucketBaseKey(piece), (c) => this.colorKey(c));
        const maxBufferSize = this.getMaxBufferSize(device);
        for (const { color, meshData, sourceBatch, sourceKey } of colorGroups.values()) {
            if (sourceKey !== null)
                for (const piece of meshData)
                    this.overlaySources.set(piece, sourceKey);
    ...
    reapplyColorOverrides(device, pipeline) {
        ...
        try {
            ...
            console.warn('[Scene] overlay rebuild after finalize failed — colours may be missing until the next repaint', err);
        }
    }
    ...
    addInstancedShard(device, shard, modelIndex = 0) {
        ...
        const lateOverriddenEids = new Set();
        for (const t of prepared) {
            ...
            for (let i = 0; i < t.instanceCount; i++) {
                ...
                if (this.instancedOverrideColors?.has(eid))
                    lateOverriddenEids.add(eid);
                if (haveBox) {
    ...
    *getInstancedEntityIds() {
     ...
     */
    setInstancedGhosting(ghostExceptIds, selectedIds, ghostAlpha) {
        const device = this.instancedDevice;
        if (!device || this.instancedTemplates.length === 0)
            return;
        ...
        for (const eid of toRestore) {
            ...
            if (override)
                this.writeInstanceColor(device, eid, override);
            else
                this.restoreInstanceColor(device, eid);
        }
     ...
     */
    getEntityLocalBounds(expressId) {
        const pieces = this.meshDataMap.get(expressId);
        if (pieces && pieces.length > 0) {
            let minX = Infinity, minY = Infinity, minZ = Infinity;
            let maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;
        ...
        if (occurrences && occurrences.length > 0) {
            ...
            for (const occ of occurrences) {
                ...
                if (!tmpl || !Number.isFinite(tmpl.localMin[0]))
                    continue;
                found = true;
                if (tmpl.localMin[0] < minX)
                    minX = tmpl.localMin[0];
                if (tmpl.localMin[1] < minY)
                    minY = tmpl.localMin[1];
                if (tmpl.localMin[2] < minZ)
                    minZ = tmpl.localMin[2];
                if (tmpl.localMax[0] > maxX)
                    maxX = tmpl.localMax[0];
                if (tmpl.localMax[1] > maxY)
                    maxY = tmpl.localMax[1];
                if (tmpl.localMax[2] > maxZ)
                    maxZ = tmpl.localMax[2];
            }
            return found ? { min: [minX, minY, minZ], max: [maxX, maxY, maxZ] } : null;
        }
        return null;
     ...
     */
    selectRect(x0, y0, x1, y1, viewportWidth, viewportHeight, viewProj, hiddenIds, isolatedIds, clip) {
        ...
        return selectBoundingBoxesInRect(this.boundingBoxes, viewProj, { x0, y0, x1, y1 }, viewportWidth, viewportHeight, hiddenIds, isolatedIds, clip);
    }
}
