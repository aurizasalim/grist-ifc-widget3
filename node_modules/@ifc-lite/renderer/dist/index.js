...
export class Renderer {
    ...
    _activePickSection = null;
    _activePickClipBox = null;
    constructor(canvas) {
        this.canvas = canvas;
    ...
    renderFrame(options) {
        ...
        const effectsLive = this.interactionEffects.frame(interacting, performance.now(), timingUnstable, options.interactionFrameIntervalMs ?? 0);
        ...
        try {
            ...
            this._activePickClipBox = options.clipBox?.enabled
                ? {
                    enabled: true,
                    min: [...options.clipBox.min],
                    max: [...options.clipBox.max],
                }
                : null;
            ...
            if (allBatchedMeshes.length > 0 || this.scene.getTexturedMeshes().length > 0 || this.scene.getInstancedTemplates().length > 0) {
                ...
                if (texturedMeshes.length > 0) {
                    ...
                    for (const tm of texturedMeshes) {
                        ...
                        if (hasVisibilityFiltering) {
                            if (options.hiddenIds?.has(tm.expressId))
                                continue;
                            if (hasIsolatedFilter && !options.isolatedIds.has(tm.expressId))
                                continue;
                        }
                        ...
                        const txAlpha = alphaForBatch({ expressIds: [tm.expressId], color: tm.color }, tm.color[3]);
                        if (txAlpha <= 0.01)
                            continue;
