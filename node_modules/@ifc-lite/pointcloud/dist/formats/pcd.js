...
export function decodePcd(buffer, 
/**
...
originOffset) {
    ...
    if (header.data !== 'binary_compressed') {
        ...
        const minBodyBytes = header.data === 'ascii' && header.pointCount > 0
            ? header.pointCount * minBytesPerPoint - 1
            : header.pointCount * minBytesPerPoint;
        if (minBytesPerPoint > 0 && minBodyBytes > availableBytes) {
            throw new Error(`PCD: declared ${header.pointCount} points need at least ` +
...
function decodeBinaryCompressed(buffer, header, originOffset) {
    ...
    if (uncompressedSize > MAX_UNCOMPRESSED_BYTES) {
        throw new Error(`PCD binary_compressed: declared uncompressed=${uncompressedSize} exceeds ` +
...
// ─── helpers ────────────────────────────────────────────────────────────────
function readScalar(view, offset, field) {
    if (field.type === 'F') {
