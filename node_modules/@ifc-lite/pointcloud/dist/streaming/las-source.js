...
import { bboxInDecodedFrame, decodeLasPoints, parseLasHeader, sampleMaxRgbChannel, } from '../formats/las.js';
...
export class LasStreamingSource {
    bytes;
    header = null;
    cursor = 0; // index of NEXT point to emit (0..header.pointCount)
    rgbScale = 1;
    downsample;
    label;
    originOffset;
    ...
    async next(maxPoints, signal) {
        ...
        if (stride === 1) {
            ...
            abortIfAborted(signal);
            const chunk = decodeLasPoints(slab, this.header, take, this.header.pointRecordLength, this.rgbScale, this.originOffset);
            this.cursor += take;
            return chunk;
        }
        ...
        if (slab.length < expectedSlabLen) {
            throw new Error(`LAS: decode expects ${expectedSlabLen} bytes for ${sourceTake} strided source records, ` +
...
//# sourceMappingURL=las-source.js.map